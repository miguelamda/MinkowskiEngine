...
# Install Ninja (uncomment the following)
...
# Or previous version of CUDA
#source /cvmfs/sft.cern.ch/lcg/releases/cuda/11.0RC-d9c38/x86_64-centos7-gcc62-opt/setup.sh

# Installation without conda (on standard Python) only works as root (uncomment the following)
#sudo su -l
